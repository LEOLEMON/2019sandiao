...
#db_path = 'F:/test/test440883.db'
#conn = sqlite3.connect(db_path)
#conn.text_factory=str
#PICPATH='F:/test/Desert.jpg'
...
def insertFJ(conn,parm1,parm2,parm3,parm4,parm5,parm6,parm7,parm8,parm9,parm10,parm11,parm12,parm13,parm14):
    ...
    conn.execute(sql_insertFJ,(parm1,parm2,parm3,parm4,parm5,parm6,parm7,parm8,parm9,parm10,parm11,parm12,parm13,parm14))
...
def main():
    arcpy.env.workspace = 'F:\\test\\sanshui20190409.gdb'
    infeature = 'SDJZ_BYZTB'

    cursor = arcpy.da.SearchCursor("SDJZ_BYZTB",
                                   ['DLBM', 'DLMC', 'QSXZ', 'QSDWDM', 'QSDWMC', 'TBMJ', 'BZ', 'SFJZ', 'XZQDM', 'TSTYBM',
                                    'ZHXGR',
                                    'SJDLBM', 'SHAPE@', 'ZLDWDM','SHAPE@XY'])

    cursorPic = arcpy.da.SearchCursor("SDJZ_TP", ['SHAPE@XY', 'FJLX', 'TSTYBM', 'FWJ', 'CJR', 'CJRQ', 'MC', 'JD', 'WD',
                                                  'CHECK_CODE'])
    listpic = []
    ...
    for row in cursor:
        ...
        for i in listpic:
            ...
            if (i['TSTYBM'] == row[9]):
                ...
                #print redictcontent
                    #soup = BeautifulSoup(redictcontent)
                    #title = soup.find('title').get_text()
                    #if (title == '404 Not Found'):
                        #urltext = soup.find('p').get_text()[18:]
                        #picurl = 'http://192.168.101.60' + urltext[:-30].replace('\\', '/')
