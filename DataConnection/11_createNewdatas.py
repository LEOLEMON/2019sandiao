...
def start(folder,xiannname,xzqdm):
    ...
    arcpy.AddMessage("11_创建文件夹路径")
    shapedir,photodir = createFolder(folder,name)

    arcpy.AddMessage("11_创建工作GDB")
    gdb = createGBD(shapedir,name)

    arcpy.AddMessage("11_创建临时GDB")
    tempgdb = createTempGBD(folder,name)

    arcpy.AddMessage("11_图层")
    DCCGTB,JZZP = createFeatureLayer(gdb,xzqdm)

    return DCCGTB,JZZP,tempgdb,photodir

if __name__ == "__main__":
    
    arcpy.AddMessage("11_创建规划院对接数据")
    ...
    DCCGTB,JZZP,tempgdb,photodir = start(folder,xiannname,xzqdm)

    arcpy.SetParameterAsText(3,DCCGTB)
    arcpy.SetParameterAsText(4,JZZP)
    arcpy.SetParameterAsText(5,tempgdb)
    arcpy.SetParameterAsText(6,photodir)

    arcpy.AddMessage("11_结束")